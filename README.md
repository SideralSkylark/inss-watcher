...
- [x] **Dry-run mode** — Flag through `Settings` to log what would happen without moving files or writing to DB. *(NF02)*
- [x] update config when something is missing (sanity check config.toml on start if something is missing add it with the default config)
